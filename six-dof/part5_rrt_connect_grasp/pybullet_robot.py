...
from pybullet_rrt import RRTPyBullet, RRTConnectPyBullet
...
class MainPyBulletRobot:
    ...
    def _path_planning(self, start_pos, end_pos):
        ...
        while True:
            ...
            # 経路生成を1度実行
            new_node_pos, near_node_pos, near_node = self._rrt.expand_once(start_pos, end_pos, self._robot.weight_joint)
            if self._is_line_interference(new_node_pos, near_node_pos):
                # 干渉あり
                self._rrt.handle_interference()
                continue

            # 干渉なしだから，ノード追加 + 経路生成の完了確認
            if not self._rrt.add_node_and_chk_goal(start_pos, end_pos, new_node_pos, near_node):
                # 終点までの近傍ではない
                continue
