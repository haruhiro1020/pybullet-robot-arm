...
class MainPyBulletRobot:
    ...
    def run(self, start_pos, path_plan):
        ...
        # 把持対象物の位置を取得
        if self.__robot.n_robot_joint == DIMENTION_2D:     # 2軸ロボットアーム
            end_pos, _ = self.__grasp_obj.get_grasp_pos(dim2=True)

        elif self.__robot.n_robot_joint == DIMENTION_3D:   # 3軸ロボットアーム
            end_pos, _ = self.__grasp_obj.get_grasp_pos()

        else:       # 6軸ロボットアーム
            end_pos, end_ori = self.__grasp_obj.get_grasp_pos()
            end_pos += end_ori
            # 逆運動学(位置から関節角度へ変換) → 6軸ロボットアームは関節空間だけの対応だから
            end_pos = self.__robot.convert_pos_to_theta(end_pos, force=True)
    ...
    def _exec_margin_time(self):
        ...
        """
        # 把持対象物の位置を取得
        end_pos, end_ori = self.__grasp_obj.get_grasp_pos(offset=False)
        if self.__robot.n_robot_joint == DIMENTION_2D:
            end_pos = np.array(end_pos)[:DIMENTION_2D]
            # 把持対象物への位置へ移動
            end_theta = self.__robot.convert_pos_to_theta(end_pos)
        elif self.__robot.n_robot_joint == DIMENTION_3D:
            end_pos = np.array(end_pos)
            # 把持対象物への位置へ移動
            end_theta = self.__robot.convert_pos_to_theta(end_pos)
        else:
            ...
            # 把持対象物への位置へ移動
            end_theta = self.__robot.convert_pos_to_theta(end_pos, force=True)
