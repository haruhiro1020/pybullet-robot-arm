...
class RRTPyBullet:
    ...
    def _strict_planning_pos(self, start_pos, end_pos):
        ...
        if self._interpolation == INTERPOLATION.POSITION.value:
            strict = self._STRICT_PLANNING_ROB_POS
        else:
            strict = self._STRICT_PLANNING_ROB_JOINT

        self._strict_min_pos = min_pos - strict
        self._strict_max_pos = max_pos + strict

        # 6軸ロボットアームの場合は関節の重みを設定
        if self._dim == DIMENTION_6D and self._interpolation == INTERPOLATION.JOINT.value:
