...
class MainPyBulletRobot:
    ...
    def _init_robot(self, robot_urdf):
        ...
        if not (self._n_joints in [DIMENTION_2D, DIMENTION_3D, DIMENTION_6D]):
    ...
    def _get_pose_limit(self):
        if self._n_joints == DIMENTION_2D:
            abs_max, n_pose_dim = 2.5, DIMENTION_3D
        elif self._n_joints == DIMENTION_3D:
            abs_max, n_pose_dim = 3.5, DIMENTION_3D
        else:
            abs_max, n_pose_dim = 3.8, DIMENTION_6D

        min_pose = np.ones(n_pose_dim) * abs_max * -1
        max_pose = np.ones(n_pose_dim) * abs_max
    ...
    def _get_init_pose(self):
        if self._n_joints == DIMENTION_2D:
            ...
            return np.array([0.5, 0.5, 1.0, 0, np.pi/2, 0])
    ...
    def _convert_pos_to_theta(self, pos):
        if self._interpolation == INTERPOLATION.POSITION.value:
            if self._n_joints in [DIMENTION_2D, DIMENTION_3D]:
                ...
                thetas = p.calculateInverseKinematics(
                    self._robot_id, self._n_joints,
                    targetPosition=pos[:DIMENTION_3D],
