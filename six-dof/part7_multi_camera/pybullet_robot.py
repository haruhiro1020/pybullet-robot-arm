...
class _PyBullet6DoFRobot(_PyBulletRobot):
    _DIM_JOINT    = DIMENTION_6D
    _DIM_POSE     = DIMENTION_6D
    _WEIGHT_JOINT = np.array([1, 1.2, 1.2, 0.2, 0.2, 0.2])
...
class PyBulletRobotController:
    ...
    def __init__(self, n_robot_joint, interpolation, hand):
        if n_robot_joint == DIMENTION_6D:
            robot_urdf = ROBOTURDF.DOF6_HAND.value if hand else ROBOTURDF.DOF6.value
